function validar(form){
    ...
    var noControl=form.noControl.value;
    ...
    mensaje+=validarnoControl(noControl);
...
function validarnoControl(cadena){

    var salida="";
    if(cadena.length!=8){
        salida='Debes informar un numero de control con 8 digitos. <br>';
    }
    return salida;
}
...
function consultarnoControl(){
    var ajax=new XMLHttpRequest();
    var noControl=document.getElementById("noControl").value;
    var url="/estudiantes/noControl/"+noControl;
    var div=document.getElementById("notificacionesNoC");
    ajax.open("get",url,true);
    ajax.onreadystatechange=function(){
        if(this.readyState==4 && this.status==200){
            var respuesta=JSON.parse(this.responseText);
